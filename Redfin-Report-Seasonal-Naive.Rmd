...
# use if just want to look at one neighborhood: for (neighborhood in c("Portland, OR - Rose City Park")){
  ...
  testData = window(oneNeighborhood.timeseries, frequency=4, start=c(2016,1), end=c(2017,10))  ### Test set    (2 years, quarterly data)
  
  # train on data from 2012 to the end of 2015
  naive.forecast <- snaive(trainingData, h = 16 )

  #Forecast for two more years (2016 and 2017) to compare to our test data
  ...
  #Quarterly plot with test data
  #plot(testData, main="Quarterly data", ylab="", xlab="Years", col = "red")  
  #lines(trainData, col="black")
  #lines(naive.forecast$mean, col="blue", lwd=3)
  #legend("topleft",lty=1,bty = "n",col=c("red","blue"),c("Test Data","Seasonal Naive"))

  # Plot Accuracy
  accuracy(naive.forecast,testData)
  tsdisplay(residuals(naive.model), main = paste(plotTitle, " Residuals"))
